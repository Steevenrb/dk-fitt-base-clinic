...
const GridMotion = ({ items = [], gradientColor = 'black' }) => {
  ...
  const totalItems = 28;
  const defaultItems = Array.from({ length: totalItems }, (_, index) => `/fondo_food.jpg`);

  // Build combinedItems safely: accept fewer images and fill the rest with defaults
  let combinedItems = [];
  if (Array.isArray(items) && items.length > 0) {
    combinedItems = items.slice(0, totalItems).map(i => i);
    if (combinedItems.length < totalItems) {
      const fill = defaultItems;
      let idx = 0;
      while (combinedItems.length < totalItems) {
        combinedItems.push(fill[idx % fill.length]);
        idx++;
      }
    }
  } else {
    combinedItems = defaultItems.slice();
  }
